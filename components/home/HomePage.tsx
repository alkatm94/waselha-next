...
const navItems = [["الرئيسية", "#home"], ["احسب التكلفة", "#calculator"], ["عنواني في الصين", "/account/china-address"], ["كيف نعمل", "#how"], ["المتاجر المدعومة", "#stores"], ["الأسئلة الشائعة", "#faq"], ["تواصل معنا", "#contact"]];
...
export function HomePage() {
  ...
  function handleImportedProduct(product: ImportedProduct) { setProductUrl(product.originalUrl); setProductName(product.title); setProductStoreName(product.storeName); setCountry(product.country); if (["USD", "CNY", "JPY", "EUR"].includes(product.currency)) setCurrency(product.currency); if (typeof product.price === "number" && product.price > 0) setPrice(String(product.price)); if (typeof product.estimatedWeightKg === "number" && product.estimatedWeightKg > 0) setWeightKg(String(product.estimatedWeightKg)); window.requestAnimationFrame(() => document.querySelector("#calculator")?.scrollIntoView({ behavior: "smooth" })); }
  return (
...
function TrustSection() {
  const cards: [string, string, typeof HandCoins][] = [
    ["رخصة ومعلومات عمل", "نوضح بيانات النشاط وقنوات التواصل قبل بدء الطلب.", ShieldCheck],
    ["دفع بعد مراجعة", "لا يتم الشراء إلا بعد إرسال السعر الكامل وموافقتك.", ReceiptText],
    ["دعم واتساب", "قناة مباشرة لإرسال الرابط واستقبال تحديثات الطلب.", ClipboardCheck],
    ["تحديثات تشغيلية", "مراحل مفهومة من الاستلام حتى الشحن والتسليم.", Truck],
  ];
  return <section id="trust" className="px-4 py-12 sm:px-6"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="الثقة أولاً" title="إشارات تشغيلية بدل عبارات تسويقية" desc="كل عنصر في الصفحة يجاوب سؤالًا عمليًا: من يراجع؟ متى أدفع؟ كيف أتابع؟ وما الذي يحدث بعد الشراء؟" /><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{cards.map(([title, desc, Icon]) => <article key={title} className="rounded-lg border border-[var(--color-border)] bg-white p-5 shadow-lg shadow-slate-900/5"><div className="grid h-12 w-12 place-items-center rounded-lg bg-[var(--color-primary)] text-[var(--color-accent)]"><Icon className="h-5 w-5" /></div><h3 className="mt-5 text-xl font-bold text-[var(--color-text)]">{title}</h3><p className="mt-2 text-[15px] font-semibold leading-7 text-[var(--color-muted)]">{desc}</p></article>)}</div></div></section>;
}
function ProcessSteps() {
  const steps = [
    ["أرسل Product Link", "انسخ رابط المنتج من المتجر وأرسله لنا مع اللون والمقاس والكمية.", Store],
    ["مراجعة المنتج", "نتحقق من التوفر، المواصفات، القيود، وخيارات الشحن.", ClipboardCheck],
    ["استلام السعر الكامل", "يصلك تفصيل واضح قبل اعتماد الطلب والدفع.", ReceiptText],
    ["الشراء", "بعد موافقتك نشتري المنتج ونتابع المورد.", HandCoins],
    ["الشحن", "نستلم الطلب ونغلفه عند الحاجة ثم نرتب الشحن الدولي.", Truck],
    ["التسليم", "تصلك التحديثات حتى وصول الطلب إلى عنوانك.", CheckCircle2],
  ] as const;
  return <section id="how" className="bg-[var(--color-surface)] px-4 py-14 sm:px-6"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="كيف نعمل؟" title="رحلة واضحة من أول رابط" desc="نحول الرابط إلى طلب قابل للمتابعة: مراجعة، سعر كامل، شراء، استلام، شحن، وتسليم." /><div className="mt-9 grid gap-3 md:grid-cols-2 lg:grid-cols-6">{steps.map(([title, desc, Icon], index) => <article key={title} className="rounded-lg border border-[var(--color-border)] bg-white p-4 shadow-sm"><div className="flex items-center justify-between gap-3"><span className="grid h-11 w-11 place-items-center rounded-lg bg-[var(--color-primary)] text-[var(--color-accent)]"><Icon className="h-5 w-5" /></span><b className="text-sm text-[var(--color-accent-dark)]">{String(index + 1).padStart(2, "0")}</b></div><h3 className="mt-5 text-lg font-bold leading-7 text-[var(--color-text)]">{title}</h3><p className="mt-2 text-sm font-semibold leading-7 text-[var(--color-muted)]">{desc}</p></article>)}</div></div></section>;
}
...
function TransparencySection() {
  const rows = [
    ["سعر المنتج", "400 USD", "1,500 ريال"],
    ["الشحن الداخلي من المتجر", "تقديري", "45 ريال"],
    ["رسوم خدمة وصلها لي", "شراء ومتابعة", "199 ريال"],
    ["الشحن الدولي", "3 KG", "126 ريال"],
    ["ضريبة القيمة المضافة", "إن وجدت", "تحدد بعد المراجعة"],
  ];
  return <section id="example-order" className="px-4 py-12 sm:px-6"><div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.82fr_1.18fr] lg:items-start"><div className="text-right"><p className="text-sm font-bold text-[var(--color-accent-dark)]">مثال طلب واقعي</p><h2 className="mt-3 text-3xl font-bold leading-tight text-[var(--color-primary-dark)] sm:text-5xl">التكلفة تظهر كفاتورة واضحة</h2><p className="mt-4 text-base font-medium leading-8 text-[var(--color-muted)]">هذا المثال يوضح طريقة التفكير في السعر: قيمة المنتج، الشحن، رسوم الخدمة، وأي رسوم تعتمد على المراجعة النهائية.</p><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-1"><div className="rounded-lg border border-[var(--color-border)] bg-white p-4 shadow-sm"><b className="text-[var(--color-primary)]">مدة تقديرية</b><p className="mt-1 text-sm font-semibold text-[var(--color-muted)]">10 - 18 يوم عمل حسب المتجر والناقل.</p></div><div className="rounded-lg border border-[var(--color-border)] bg-white p-4 shadow-sm"><b className="text-[var(--color-primary)]">ملاحظة مهمة</b><p className="mt-1 text-sm font-semibold text-[var(--color-muted)]">السعر النهائي يعتمد على رابط المنتج والوزن الفعلي.</p></div></div></div><article className="rounded-lg border border-[var(--color-border)] bg-white shadow-2xl shadow-slate-900/8"><div className="flex flex-col gap-3 border-b border-[var(--color-border)] p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-bold text-[var(--color-muted)]">WASELHALI QUOTE</p><h3 className="mt-1 text-2xl font-bold text-[var(--color-primary-dark)]">ملخص تكلفة طلب</h3></div><span className="inline-flex w-fit items-center gap-2 rounded-lg bg-emerald-50 px-3 py-2 text-sm font-bold text-emerald-700"><CheckCircle2 className="h-4 w-4" />مثال توضيحي</span></div><div>{rows.map(([label, meta, value]) => <div key={label} className="grid grid-cols-[1fr_auto] gap-4 border-b border-[var(--color-border)] px-5 py-4 last:border-b-0"><div><p className="text-base font-bold text-[var(--color-text)]">{label}</p><p className="mt-1 text-sm font-semibold text-[var(--color-muted)]">{meta}</p></div><b className="text-left text-base text-[var(--color-primary-dark)]">{value}</b></div>)}</div><div className="border-t border-[var(--color-border)] bg-[var(--color-primary-dark)] p-5 text-white"><div className="flex items-center justify-between gap-4"><span className="text-base font-bold text-white/75">الإجمالي قبل الرسوم المتغيرة</span><strong className="text-4xl font-bold text-[var(--color-accent)]">1,870 <span className="text-lg text-white">ريال</span></strong></div><p className="mt-4 rounded-lg border border-white/10 bg-white/[0.06] p-4 text-sm font-semibold leading-7 text-white/75">هذا مثال فقط. السعر النهائي يعتمد على المنتج، التوفر، الوزن الفعلي، بلد الشحن، وأي رسوم مطبقة.</p></div></article></div></section>;
}
function FAQSection() {
...
function FloatingWhatsApp({ onClick }: { onClick: () => void }) {
  return <button onClick={onClick} className="fixed bottom-4 left-4 z-40 grid h-12 w-12 sm:h-14 sm:w-14 place-items-center rounded-full bg-[#25D366] text-[#062d18] shadow-xl shadow-black/20 transition hover:-translate-y-0.5" aria-label="تواصل عبر واتساب"><MessageCircle className="h-6 w-6" /></button>;
}
...
function SectionHeading({ eyebrow, title, desc, inverse = false, align = "center" }: { eyebrow: string; title: string; desc: string; inverse?: boolean; align?: "center" | "right" }) {
  return <div className={align === "center" ? "mx-auto max-w-3xl text-center" : "max-w-3xl text-right"}><p className={`text-sm font-bold ${inverse ? "text-[var(--color-accent)]" : "text-[var(--color-accent-dark)]"}`}>{eyebrow}</p><h2 className={`mt-3 text-2xl font-bold leading-tight sm:text-5xl ${inverse ? "text-white" : "text-[var(--color-text)]"}`}>{title}</h2><p className={`mt-4 text-base font-medium leading-8 ${inverse ? "text-white/68" : "text-[var(--color-muted)]"}`}>{desc}</p></div>;
}
